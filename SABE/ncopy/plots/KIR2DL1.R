	...
	db <- read.table('KIR2DL1_plot_db.txt',h=T,sep='\t')
	dbsub1 <- subset(db, Gene == 'KIR2DL1')
	dbsub2 <- subset(db, Gene == 'KIR2DP1')
	...
	p <- ggplot() + 
		...
		labs(title='KIR2DL1', caption = 'Calculated using the wgs mode')
	
	ggsave("/media/lab/Data/kir-mapper_SABE/output//ncopy//plots//KIR2DL1.png", width=8, height=6,bg = "white")
	...
	saveWidget(myplot, '/media/lab/Data/kir-mapper_SABE/output//ncopy//plots//KIR2DL1.html', selfcontained = F, libdir = '/media/lab/Data/kir-mapper_SABE/output//ncopy//plots//KIR2DL1')