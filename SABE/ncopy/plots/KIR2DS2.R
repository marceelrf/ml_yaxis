	...
	db <- read.table('KIR2DS2_plot_db.txt',h=T,sep='\t')
	dbsub1 <- subset(db, Gene == 'KIR2DS2')
	dbsub2 <- subset(db, Gene == 'KIR2DL2')
	...
	p <- ggplot() + 
		...
		labs(title='KIR2DS2', caption = 'Calculated using the wgs mode')
	
	ggsave("/media/lab/Data/kir-mapper_SABE/output//ncopy//plots//KIR2DS2.png", width=8, height=6,bg = "white")
	...
	saveWidget(myplot, '/media/lab/Data/kir-mapper_SABE/output//ncopy//plots//KIR2DS2.html', selfcontained = F, libdir = '/media/lab/Data/kir-mapper_SABE/output//ncopy//plots//KIR2DS2')