	...
	db <- read.table('KIR2DL2_plot_db.txt',h=T,sep='\t')
	dbsub1 <- subset(db, Gene == 'KIR2DL2')
	dbsub2 <- subset(db, Gene == 'KIR2DL3')
	...
	p <- ggplot() + 
		...
		labs(title='KIR2DL2', caption = 'Calculated using the wgs mode')
	
	ggsave("/media/lab/Data/kir-mapper_SABE/output//ncopy//plots//KIR2DL2.png", width=8, height=6,bg = "white")
	...
	saveWidget(myplot, '/media/lab/Data/kir-mapper_SABE/output//ncopy//plots//KIR2DL2.html', selfcontained = F, libdir = '/media/lab/Data/kir-mapper_SABE/output//ncopy//plots//KIR2DL2')